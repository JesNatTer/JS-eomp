...
fetch(`https://js-backend.herokuapp.com/viewprofile/${username}`)
...
.then(data => {
    ...
    if (data['data'] == ''){
        ...
        data['data'].forEach((detail) => {
            ...
            fetch(`https://js-backend.herokuapp.com/myproducts/${detail[0]}/`, {
                method: "get",
                headers: {
...
function edititem() {
    ...
    fetch(`https://js-backend.herokuapp.com/edit-product/${productid}/`, {
        method: 'PUT',
        headers: {
            'Content-Type' : 'application/json',
            'Authorization' : `jwt ${mystorage.getItem('jwt-token')}`
...
function deleteitem(){
    fetch(`https://js-backend.herokuapp.com/delete-product/${productid}`, {
        headers: {
            'Authorization' : `jwt ${mystorage.getItem('jwt-token')}`
        },
